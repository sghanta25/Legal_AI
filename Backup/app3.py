...
def send_legal_mail():
    ...
    if not issue or not recipient_email:
        return jsonify({'error': 'Please provide both the issue and recipient email.'}), 400

    if not EMAIL_USER or not EMAIL_PASSWORD:
        return jsonify({'error': 'Email credentials not configured on the server.'}), 500

    # Step 1: Use the LLM to draft the legal notice
    drafting_prompt = f"""
    You are a legal letter drafting assistant. Draft a formal legal notice based on the following problem.
    The notice should be firm but professional, outlining the issue, the action required from the recipient,
    and a reasonable deadline for compliance. Do not use placeholders for personal information.
    ...
    try:
        llm = Ollama(model="llama3.2")
        legal_notice_body = llm.invoke(drafting_prompt)

        # Step 2: Construct the email message
        msg = MIMEMultipart()
        msg['From'] = EMAIL_USER
        msg['To'] = recipient_email
        msg['Subject'] = "LEGAL NOTICE REGARDING YOUR ACTIONS"
        
        # Attach the body of the legal notice
        msg.attach(MIMEText(legal_notice_body, 'plain'))
        ...
        return jsonify({'message': '✅ Legal notice sent successfully!'}), 200
